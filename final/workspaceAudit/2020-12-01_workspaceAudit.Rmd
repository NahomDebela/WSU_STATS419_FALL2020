---
title: 'R Notebook: Workspace Audit for [nahom.debela]'
output:
  html_document:
    df_print: paged
    toc: true
...
```{r}
path.to.git.local = "C:/Users/nahom/_git_/WSU_STATS419_FALL2020/";
setwd(path.to.git.local);
path.to.git.final = paste0(path.to.git.local,"final/workspaceAudit/");
```
...
I deserve 45/50 points because I managed to commit my repository multiple times. Of course I could have commited more frequently but I didn't see the point because I didn't have many new files being updated in that folder and I decided that I would simply commit the files when necessary. I was using git bash for another self project I had (creating github portfolio) so I dont think the issue if any is effort or use of git bash. As far as consistency, I used it when I deemed necessary which is why there are about 3 dates I committed to my WSU repository.

My work hours are not consistent but to me that is expected as I dont have a "time" schedule for when to work on certain things. I have a weekly schedule of all assignments I need to submit that week.

During the semester I had a couple of issues with R which lead me to update all my packages more than once. I also had to re-install R and update MikTex to get the Latex working.

When it comes to dropbox, I did not have it downloaded all semester. I recently just downloaded it to complete this workspace Audit. I honestly didn't know it was a requirement or that we would be graded on our use of it. When I heard dropbox was an option, I decided not to use it and to simply check manually for updates. I don't think my lack of consistency and use of certain workspace tools such as dropbox will be too detrimental, which is why I chose to mark down 5 total points. Furthermore, I do believe I have a good understanding of these tools and why theyre important, especially github/version control. Ultimately, I would label myself as approaching Mastery and give myself a 45/50. Thank you.
</pre>
